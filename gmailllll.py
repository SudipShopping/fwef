...
ADMIN_BOT_TOKEN = "8272656277:AAEUXGW5MMWwgWuk7W388ntIXFS0nEGniZ8" 
PUBLIC_BASE_URL = "https://tweet2telegram-nqgw.onrender.com" 
...
def sb_upsert(table, data_list, on_conflict=None):
    ...
    try:
        ...
        r = requests.post(url, headers=headers, json=data_list, timeout=20)
...
def set_webhook(bot_token, path):
    ...
    try:
        r = requests.get(url, params={"url": full}, timeout=20)
...
# 🔧 FIX 4: Improved Debug Logging for Upsert
def sb_upsert_account(uid: int, keys: dict) -> bool:
    ...
    try:
        ...
        if r.status_code in (200, 201):
            ...
            print(f"\n✅ SUCCESS! Account @{username} successfully updated/created for user {uid}")
            print(f"{'='*60}\n")
            return True
        else:
            print(f"\n❌ FAILED!")
...
# --- Local Fallback/Primary DB Helpers ---
...
# 
# 🔧 FIX 2: Update Tweet Link Format with Random Token
#
def post_tweet_to_x(tweet_id, user_id, account_username, tweet_text):
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            if tweet_id_str:
                # ✅ FIX: Generate Random Token for Link
                random_token = secrets.token_urlsafe(16)[:12]
                tweet_link = f"https://x.com/{account_username}/status/{tweet_id_str}?t={random_token}&s=08"
    ...
    try:
        if "callback_query" in update:
            ...
            if state == "schedule_flow_ampm":
                ...
                requests.post(f"{MAIN_BOT_API}/answerCallbackQuery", json={"callback_query_id": cb["id"]})
...
def main_bot_flow_continue(uid, chat_id, text, state, flow_data, is_callback=False, callback_update=None):
    ...
    if state == "schedule_flow_ampm":
        ...
        if is_callback and callback_update:
            requests.post(f"{MAIN_BOT_API}/answerCallbackQuery", json={"callback_query_id": callback_update["callback_query"]["id"]})
...
def handle_access_key(uid, chat_id, key):
    try:
        ...
        if row:
            ...
            if not updated:
                print(f"Key update failed for key: {key}") 
    ...
    try:
        ...
        for i in range(num_to_schedule):
            ...
            try:
                ...
                r = requests.patch(url, headers=SB_HEADERS, json=update_data, timeout=20)
...
def check_and_post_scheduled_tweets():
    ...
    NO rate limiting - posts immediately.
    """
    ...
    for i, tweet in enumerate(tweets_to_post, 1):
        ...
        print(f"\n📤 [{i}/{len(tweets_to_post)}] ID:{tweet_id} → @{account_username}")
        ...
        if post_link:
            ...
            try:
                ...
                if r.status_code == 200:
                    link_rows = r.json()
                    print(f"    🔍 Link Bot query for user {tg_id}: Found {len(link_rows)} connections")
                    
                    for row in link_rows:
                        chat_id = row.get('link_bot_chat_id')
                        
                        if chat_id:
                            print(f"    📤 Sending link to Link Bot chat: {chat_id}")
                            
                            success, status = send_msg(LINK_BOT_API, chat_id, post_link)
                            
                            if success:
                                print(f"    ✅ Link sent successfully to chat {chat_id}")
                            else:
                                print(f"    ❌ Failed to send to chat {chat_id}: {status}")
                else:
                    print(f"    ⚠️ Link Bot query failed: Status {r.status_code}")
                    
            except Exception as e:
                print(f"    ⚠️ Link bot notify failed: {e}")
                traceback.print_exc()
...
# 🔧 FIX 3: Fixed `uid` variable name error (changed to `tg_id`)
...
def api_update_account(tg_id):
    ...
    # ✅ FIX: Use tg_id (from decorator), not uid (which was undefined)
...
def api_delete_tweet(tg_id):
    ...
    try:
        ...
        # ✅ FIX: Clear return logic
        if deleted_count > 0:
            return jsonify({"status": "ok", "message": "Tweet deleted successfully"}), 200
        else:
...
def test_supabase_insert():
    ...
    try:
        url = f"{SUPABASE_URL}/rest/v1/{SB_TABLE_ACCOUNTS}"
        r = requests.post(url, headers=SB_HEADERS, json=test_payload, timeout=10)
        results.append({
            "test": "INSERT",
            "status": r.status_code,
            "response": r.text,
        ...
        r_del = requests.delete(delete_url, headers=SB_HEADERS, timeout=10)
        results.append({
            "test": "Cleanup",
